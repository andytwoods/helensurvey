...
# DATABASES
# ------------------------------------------------------------------------------
# base.py falls back to SQLite when no database env vars are present; production
# must never run on that fallback, so require a real DATABASE_URL here.
env("DATABASE_URL")
...
DEFAULT_FROM_EMAIL = env(
    ...
    default="HelenSurvey <noreply@example.com>",
)
...
# EMAIL BACKEND
# ------------------------------------------------------------------------------
# This project deliberately runs without an email system: the quiz is anonymous
# and staff sign in through the Django admin, so nothing needs to be delivered.
# Anything that does try to send is written to the container log rather than
# silently dropped.
...
# No `mail_admins` handler: there is no email system here (see above), and
# Django's default `django` logger is redefined so error records cannot
# propagate into the built-in one that still carries `mail_admins`.
LOGGING = {
    "version": 1,
